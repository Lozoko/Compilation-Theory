...
class Mparser(object):
    ...
    def p_idx_assign(self, p):
        ...
        if len(p) == 4:
            ...
            p[0] = AST.IdxTwoDimensionTableAssignBySlicing(p[1], p[3], p[5], p[7], p[9], p[12])
        p[0].line_no = self.scanner.lexer.lineno
    ...
    def p_printtriangle(self, p):
        """PRINTTRIANGLE : PRINT STRING '*' EXPRESSION """
        p[0] = AST.PrintTriangle(p[2], p[4])
        p[0].line_no = self.scanner.lexer.lineno
    ...
    def p_printmany(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = AST.PrintManyTable(p[1], p[2], p[4])
        elif len(p) == 3:
            p[0] = AST.PrintTable(p[1], p[2])
        else:
